...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        grads['W2'] = np.dot(a.T, scores_prob) / num_train + 2*reg*W2
        grads['b2'] = np.sum(scores_prob, axis=0) / num_train
        grads['W1'] = np.dot(X.T, np.dot(scores_prob, W2.T) * (a>0)) / num_train + 2*reg*W1
        grads['b1'] = np.sum(np.dot(scores_prob, W2.T) * (a>0), axis=0) / num_train
